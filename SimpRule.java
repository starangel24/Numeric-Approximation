...
public class SimpRule extends FuncInfo{

	private double delta;
	
    public SimpRule(int a, int b, int n) {
		super (a,b,n);
   ...
   /* public double getApproxArea1 (){
    	...
    	for (int i=0; i<temp.length; i++){
    		double num= temp[i];
    		approxArea= approxArea+(((Math.pow(num,5)))+ (7*(Math.pow(num,2))))*((double) temp2[i]);	
    	}
    	return approxArea*((double) delta/3);
    }
    */
    public double getApproxArea1 (){
    	double approxArea=0.0;
    ...
    public String toString(){
    	...
    	sb.append("This is for Simpson Rule: "+ "\n");
